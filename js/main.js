...
const widgets = [
  {
    ...
    configs: [
      ...
      { key: 'bg', label: '배경색', type: 'color', default: '#000000' }
    ]
  },
  {
    id: 'clock',
    ...
    configs: [
      ...
      { key: 'theme', label: '테마', type: 'select', options: ['dark', 'light'] },
  ...
  },
  {
    ...
    icon: '⏳',
    name: '카운트다운 타이머',
    desc: '특정 날짜까지 남은 시간 표시',
    configs: [
      { key: 'target', label: '목표 날짜', type: 'date' },
      { key: 'title', label: '제목', type: 'text', placeholder: 'D-Day' },
      { key: 'theme', label: '테마', type: 'select', options: ['dark', 'light', 'gradient'] },
      { key: 'color', label: '강조 색상', type: 'color', default: '#6c5ce7' }
    ]
  },
  {
    id: 'progress-bar',
    ...
    configs: [
      ...
      { key: 'color', label: '바 색상', type: 'color', default: '#6c5ce7' }
    ]
  },
  {
    id: 'quote',
    icon: '💬',
    name: '명언 / 인용구',
    desc: '커스텀 문구 또는 랜덤 명언 표시',
    configs: [
      { key: 'text', label: '문구 (비우면 랜덤)', type: 'text', placeholder: '오늘도 화이팅!' },
      { key: 'author', label: '저자', type: 'text', placeholder: '' },
      { key: 'theme', label: '테마', type: 'select', options: ['dark', 'light', 'minimal'] },
      { key: 'font', label: '폰트 스타일', type: 'select', options: ['serif', 'sans', 'handwriting'] }
    ]
  },
  {
    id: 'image-slider',
  ...
  grid.innerHTML = widgets.map(w => `
    <div class="widget-card" id="card-${w.id}">
      ...
      <div class="widget-actions">
        <button class="btn btn-config" onclick="toggleConfig('${w.id}')">⚙️ 설정</button>
